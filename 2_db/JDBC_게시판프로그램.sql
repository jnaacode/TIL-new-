...
-- [관리자 계정] member 계정 생성
CREATE USER lys_member IDENTIFIED BY member1234;

-- [관리자 계정] CONNECT, RESOURCE + CREATE VIEW 권한 부여 + 객체 생성 공간 할당
GRANT CONNECT, RESOURCE, CREATE VIEW TO lys_member;
ALTER USER lys_member DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- lys_member 계정 접속 방법 추가
...
CREATE TABLE REPLY(
    ...
    CREATE_DATE DATE DEFAULT SYSDATE,
    MEMBER_NO NUMBER REFERENCES MEMBER,   -- MEMBER 테이블 PK 잠조
    BOARD_NO NUMBER REFERENCES BOARD   -- BOARD 테이블 PK 잠조
...
-- 외래키 제약조건 삭제 및 ON DELETE CASCADE 추가 > 부모키 삭제시 자식키도 삭제
ALTER TABLE REPLY DROP CONSTRAINT SYS_C007614;
ALTER TABLE BOARD DROP CONSTRAINT SYS_C007617;

ALTER TABLE BOARD ADD FOREIGN KEY(MEMBER_NO) ;

ALTER TABLE BOARD ADD FOREIGN KEY(MEMBER_NO) REFERENCES MEMBER(MEMBER_NO);
ALTER TABLE REPLY ADD FOREIGN KEY(MEMBER_NO) REFERENCES BOARD(BOARD_NO) ON DELETE CASCADE;


-- 게시글 삭제

DELETE FROM BOARD WHERE BOARD_NO = 1;
-- ORA-02292 : 무결성 제약조건(LYS MEMBER.SYS_C007613)이 위배되었습니다 - 자식 레코드가 발견되었습니다.
...
ALTER TABLE REPLY DROP CONSTRAINT SYS_C007616;
