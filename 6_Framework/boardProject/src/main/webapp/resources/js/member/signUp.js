...
// 2) 객체에 특정 Key가 존재하지 않으면 추가 할 수 있다. 
// ex) const obj = {"a":1, "b":2}
//     obj.c = 3 -> {"a":1, "b":2, "c":3}

// 3) 객체에 특정 key를 삭제 할 수 있다 (delete 연산자)
// ex) const obj = {"a":1, "b":2}
//     delete obj.b  -> {a: 1, c: 3}

// 회원 가입 JS

/* 유효성 검사 진행 여부 확인 용 객체 */
...
memberPw.addEventListener("input", () => {
    ...
    if (memberPw.value.trim().length == 0) {
        memberPw.value = ""; //띄어쓰기 제거
        pwMessage.innerText = "영어,숫자,특수문자(!,@,#,-,_) 6~20글자 사이로 입력해주세요.";
        pwMessage.classList.remove("error", "confirm"); // 검정글씨
...
memberNickname.addEventListener("input", () => {
    ...
    if (memberNickname.value.trim().length == 0) {
        ...
        nickMessage.innerText = "한글,영어,숫자로만 2~10글자만 입력해주세요.";
    ...
    if (regEx.test(memberNickname.value)) { // 유효
        ...
        fetch("/dubCheck/nickname?nickname="+memberNickname.value)
        .then( resp => resp.text())
        .then( count =>{

            if(count == 0){ //중복이 아닌 경우 
                nickMessage.innerText = " 사용가능한 닉네임니다. ";
        ...
        nickMessage.innerText = "유효한 형식이 아닙니다. ";
        nickMessage.classList.add("error");
        nickMessage.classList.remove("confirm");
...
memberTel.addEventListener("input", () => {
    ...
    if (regEx.test(memberTel.value)) {
        ...
        telMessage.innerText = "유효한 형식이 아닙니다. ";
...
checkAuthKeyBtn.addEventListener("click", function(){
    ...
    if(authMin > 0 || authSec > 0){ // 시간 제한이 지나지 않은 경우에만 인증번호 검사 진행
        ...
        //  query 예시 =  inputKey=12345&email=user01
...
document.getElementById("signUpFrm").addEventListener("submit", e => {
    ...
    // 배열용(for문)
    // for ... of : 향상된 for문 
    //            -> iterator(반복자) 속성을 지닌 배열, 유사 사용 가능

    // 객체용(for문)
    // for .. in : 
    //          -> js 객체가 가지고 잇는 key를 순서대로 하나씩 꺼내는 반복문
    ...
    for (let key in checkObj) {
        ...
        if (!checkObj[key]) { // 각 key에 대한 value(true/false)를 얻어와 false인 경우 유효하지 않다!
            ...
            switch (key) {
                ...
                case "memberPwConfirm": alert("비밀번호 확인되지 않습니다."); break;
