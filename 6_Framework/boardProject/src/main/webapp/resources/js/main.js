...
if (loginFrm != null) {
    ...
    loginFrm.addEventListener("submit", e => {
        ...
        if (memberEmail.value.trim().length == 0) {
            ...
            memberEmail.innerHTML = ""; // 잘못 입력된 값(공백) 제거
            memberEmail.focus(); // 이메일 input 태그에 초점 맞춤
...
btn3.addEventListener("click", ()=>{
    ...
    fetch("/selectMemberList",{
        ...
        body : input.value // 보내질 문자열 하나(이렇게 잘 작성 안한다!)
    ...
    .catch( err => {
        console.log(err)
        // result3.innerHTML = 이부분은 오류 발생했을때 보이는 곳이라서 작성하지 않는다.
    })
