...
(function(){
    ...
    if(goToListBtn != null){ // 목록으로 버튼이 화면에 있을 때만 이벤트 추가
        ...
        goToListBtn.addEventListener("click", function(){
            ...
            url += "/board/list?" // /comunitty/board/list?
...
(function(){
    ...
    if(deleteBtn != null){ // 버튼이 화면에 존재할 때
        deleteBtn.addEventListener("click", function(){
            // 현재 : detail?no=1603&cp=1&type=1

            // 목표 : delete?no=1603&type=1
            ...
            const no = "?no=" + params.get("no"); // ?no=1603
            
            const type = "&type=" + params.get("type"); // &type=1

            // url에 쿼리스트링 추가
            url += no + type; // delete?no=1603&type=1
